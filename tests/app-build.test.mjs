...
test("packages the Gemini-only workbook-grounded Ask endpoint and focused context policy", async () => {
  ...
  assert.equal(payload.service_tier, "standard");
...
test("answers a high-confidence Excel lookup directly without a generative-model call", async () => {
...
test("routes Ask AI fallback through Gemini with the same hydrated Excel evidence", async () => {
  ...
  try {
    ...
    assert.equal(payload.answer.model, "gemini-3.7-flash");
