...
The generation model is `gpt-5.6-sol` with fixed `medium` reasoning in Standard mode. `gpt-5.6-terra` is used only as the same-generation fallback for a transient Sol rate limit. Official [GPT-5.6 model guidance](https://developers.openai.com/api/docs/guides/latest-model) describes medium as the balanced starting point. The reusable instruction prefix stays stable for [prompt caching](https://developers.openai.com/api/reference/resources/responses/methods/create), while the changing question and compact retrieved evidence are appended per request.
