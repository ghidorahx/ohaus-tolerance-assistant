...
The Google Search preview includes a separate **Google AI Test** tab. Its `/api/google-direct` endpoint uses `gemini-3.8-flash` with low thinking, Google Search and URL context, without calling D1, Vectorize, or the catalog assistant. It sends at most four recent turns from this tab, caps output at 6,000 tokens, and accepts factual answers only with citations on the final model output and successful search or matching URL retrieval. Inline citations link to their sources; reference details stay collapsed. All supplied Search Suggestions are displayed with the associated result.

Ambiguous questions use a constrained clarification function: the app renders a question from an allowlisted missing-detail enum, never uncited model claims. An explicit unverified-result function is accepted only after research; the app never turns missing evidence into a guessed specification. Incomplete/uncited results and transient failures get at most one focused retry (two requests total, 25 seconds per attempt, 60 seconds overall). Failed validation returns a cautious app-authored response; provider failures remain distinct. Structured diagnostics contain only status, reason enums and counts, not prompts, answers, source contents or credentials. Stop cancels a pending browser request.
