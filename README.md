...
The Google Search preview includes a separate **Testing** tab. Its `/api/google-direct` endpoint uses `gemini-3.8-flash` with low thinking, Google Search and URL context, without calling D1, Vectorize, or the catalog assistant. It sends at most four recent turns from this tab, caps output at 6,000 tokens, and accepts factual answers only with citations on the final model output and successful search or matching URL retrieval. The chat omits descriptions, source panels, and model/timing details. Small inline citation links and all supplied Search Suggestions remain with the associated result.
