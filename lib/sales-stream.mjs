...
/** @param {(draft: (text: string) => void, signal: AbortSignal, status: (message: string) => void) => Promise<Response>} run
      ...
      void (async () => {
        try {
          ...
          const response = await run((text) => {
            ...
            const bounded = String(message ?? "").trim().slice(0, 180);
            if (bounded) emit("status", { message: bounded });
          });
