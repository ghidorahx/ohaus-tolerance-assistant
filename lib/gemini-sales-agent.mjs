...
import { readEvents, answerPrefix } from "./sales-stream.mjs";
  ...
  if (response.ok && body.stream) {
    ...
    for await (const event of readEvents(response.body)) {
      ...
      if (event.event_type === "step.delta" && event.index === outputIndex && event.delta?.type === "text") {
        ...
        if (raw.length > 262144) throw new Error("Gemini answer too large.");
        const draft = answerPrefix(raw);
        if (draft !== previous) { onDraft(draft); previous = draft; }
      }
      if (event.event_type === "interaction.completed") return { ...event.interaction, output_text: raw };
      if (event.event_type === "error" || event.event_type === "interaction.failed") throw new Error("Gemini stream failed.");
    }
    throw new Error("Gemini stream ended without a complete answer.");
  }
 ...
 * @param {Array<{question: string, answer: string, materials: string[], contextSummary: string}>=} options.sessionContext
 * @param {string} options.apiKey
 * @param {string=} options.model
 * @param {string=} options.thinkingLevel
 * @param {Record<string, unknown>} options.groundingBundle
 * @param {((items: unknown, groundingBundle: object) => Array<Record<string, unknown>> | Promise<Array<Record<string, unknown>>>)=} options.evidenceHydrator
 * @param {typeof fetch=} options.fetchImpl
 * @param {((text: string) => void)=} options.onDraft
