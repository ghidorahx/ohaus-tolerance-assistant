...
export const MAX_RETRIEVAL_DOCUMENTS = 8;
const MAX_CATALOG_SEARCH_RESULTS = 40;
const byMaterial = new Map(records.map((record) => [String(record.material_number), record]));
const retrievalDocumentById = new Map(retrievalIndex.documents.map((document) => [document.document_id, document]));
const productsByModel = new Map();
const inboundRelationshipsByMaterial = new Map();
...
for (const product of products) {
  ...
  for (const [relationshipType, materialNumbers] of Object.entries(product.relationships ?? {})) {
    for (const materialNumber of materialNumbers ?? []) {
      const material = String(materialNumber);
      const inbound = inboundRelationshipsByMaterial.get(material) ?? [];
      inbound.push({
        relationship_type: relationshipType,
        source_material_number: String(product.material_number),
      });
      inboundRelationshipsByMaterial.set(material, inbound);
    }
  }
}
...
const customerIntentConcepts = [
  {
    id: "battery_operation",
    patterns: [
      /\b(?:battery|batteries|battery powered|cordless)\b/i,
      /\b(?:no|without|away from|there (?:may|might) not be) (?:a |an |any )?(?:nearby )?(?:outlet|power|electricity)\b/i,
      /\b(?:job ?site|in the field|on the go|off grid)\b/i,
    ],
    catalog_terms: ["battery", "portable", "mobile", "outdoor"],
    selection: { battery_required: true },
  },
  {
    id: "industrial_use",
    patterns: [/\b(?:warehouse|factory|production floor|shop floor|industrial work|manufacturing)\b/i],
    catalog_terms: ["Work in Industrial", "industrial", "production", "warehouse"],
    selection: { usage_context: ["Work in Industrial"] },
  },
  {
    id: "education_use",
    patterns: [/\b(?:classroom|school|student|teaching lab|education)\b/i],
    catalog_terms: ["Classroom", "education", "school"],
    selection: { usage_context: ["Classroom"] },
  },
  {
    id: "parts_counting",
    patterns: [
      /\b(?:parts?|pieces?|components?|screws?|bolts?|tablets?|pills?) count(?:ing)?\b/i,
      /\bcount(?:ing)?(?: [a-z]+){0,2} (?:parts?|pieces?|components?|screws?|bolts?|tablets?|pills?)\b/i,
      /\binventory count(?:ing)?\b/i,
    ],
    catalog_terms: ["Parts Counting", "counting"],
    selection: { applications: ["Parts Counting"] },
  },
  {
    id: "check_weighing",
    patterns: [
      /\b(?:check ?weighing|pass fail|over under|within (?:a )?target|target weight|portion control)\b/i,
      /\b(?:too heavy|too light)\b/i,
    ],
    catalog_terms: ["Check Weighing", "Checkweighing", "target weight"],
    selection: { applications: ["Check Weighing", "Checkweighing"] },
  },
  {
    id: "dynamic_weighing",
    patterns: [/\b(?:animal weighing|live animals?|moving samples?|unstable samples?|dynamic weighing)\b/i],
    catalog_terms: ["Animal/Dynamic Weighing", "dynamic weighing"],
    selection: { applications: ["Animal/Dynamic Weighing"] },
  },
  {
    id: "density_determination",
    patterns: [/\b(?:density|specific gravity)\b/i],
    catalog_terms: ["Density Determination", "density"],
    selection: { applications: ["Density Determination"] },
  },
  {
    id: "display_hold",
    patterns: [
      /\b(?:display hold|hold (?:the )?(?:display|reading)|freeze (?:the )?(?:display|reading|number))\b/i,
      /\b(?:reading|number|result) (?:to )?(?:stay|remain) (?:on|visible|put)\b/i,
    ],
    catalog_terms: ["Display Hold", "hold reading"],
    selection: { applications: ["Display Hold"] },
  },
  {
    id: "compact_space",
    patterns: [
      /\b(?:small footprint|limited (?:bench|counter|table) space|tight (?:bench|counter|table) space|cramped)\b/i,
      /\b(?:easy to store|stackable|tuck (?:it )?away|doesn'?t take up much space)\b/i,
    ],
    catalog_terms: ["small", "portable", "slim", "stackable", "space saving"],
    selection: { search_terms: ["small", "portable", "slim", "stackable", "space saving"] },
  },
  {
    id: "fine_measurement",
    patterns: [
      /\b(?:fine increments?|tiny (?:changes|amounts)|small (?:changes|differences)|high precision|very precise)\b/i,
      /\b(?:accurate|accuracy|resolution)\b/i,
    ],
    catalog_terms: ["readability", "resolution", "increment", "precision"],
    selection: {},
  },
  {
    id: "data_connectivity",
    patterns: [
      /\b(?:send|export|transfer|download) (?:the )?(?:weight|reading|result|data)s?\b/i,
      /\b(?:connect|connection) to (?:a )?(?:computer|pc|printer|system)\b/i,
    ],
    catalog_terms: ["interface", "communication", "USB", "RS232", "printer"],
    selection: { search_terms: ["interface", "communication", "USB", "RS232", "printer"] },
  },
  {
    id: "wet_environment",
    patterns: [/\b(?:washdown|wet area|gets? splashed|water resistant|waterproof)\b/i],
    catalog_terms: ["washdown", "wet", "water protection"],
    selection: { search_terms: ["washdown", "wet", "water protection"] },
  },
  {
    id: "legal_for_trade",
    patterns: [/\b(?:legal for trade|trade approved|selling by weight|sell by weight|commercial transaction)\b/i],
    catalog_terms: ["legal for trade", "trade approved"],
    selection: { legal_for_trade: "Legal for Trade" },
  },
];
...
export function interpretCustomerQuestion(question) {
  const originalQuery = String(question ?? "").trim();
  const matches = customerIntentConcepts.filter((concept) => (
    concept.patterns.some((pattern) => pattern.test(originalQuery))
  ));
  const catalogTerms = [...new Set(matches.flatMap((concept) => concept.catalog_terms))];
  const derivedSelection = {
    applications: [...new Set(matches.flatMap((concept) => concept.selection.applications ?? []))],
    usage_context: [...new Set(matches.flatMap((concept) => concept.selection.usage_context ?? []))],
    search_terms: [...new Set(matches.flatMap((concept) => concept.selection.search_terms ?? []))],
    battery_required: matches.some((concept) => concept.selection.battery_required === true) ? true : null,
    legal_for_trade: matches.find((concept) => concept.selection.legal_for_trade)?.selection.legal_for_trade ?? null,
  };

  return {
    original_query: originalQuery,
    expanded_query: [originalQuery, ...catalogTerms].filter(Boolean).join(" "),
    recognized_concepts: matches.map((concept) => ({
      id: concept.id,
      catalog_terms: concept.catalog_terms,
    })),
    derived_selection: derivedSelection,
  };
}
...
function semanticRetrievalDocuments({ query, semanticRetrieval }) {
  const lexical = searchRetrievalDocuments({
    query,
    document_type: "any",
    limit: MAX_RETRIEVAL_DOCUMENTS,
  });
  const vectorMatches = semanticRetrieval?.status === "ready" && Array.isArray(semanticRetrieval.matches)
    ? semanticRetrieval.matches
    : [];
  const tokens = tokenize(query);
  const selected = [];
  const selectedIds = new Set();
  ...
  for (const match of vectorMatches) {
    if (selected.length >= MAX_RETRIEVAL_DOCUMENTS) break;
    const document = retrievalDocumentById.get(String(match?.document_id ?? ""));
    if (!document || selectedIds.has(document.document_id)) continue;
    selectedIds.add(document.document_id);
    selected.push({
      score: Math.round((Number(match.score) || 0) * 1_000),
      semantic_score: Number(match.score) || 0,
      document_id: document.document_id,
      document_type: document.document_type,
      title: document.title,
      material_number: document.material_number,
  ...
  for (const result of lexical.results) {
    if (selected.length >= MAX_RETRIEVAL_DOCUMENTS) break;
    if (selectedIds.has(result.document_id)) continue;
    selectedIds.add(result.document_id);
    selected.push(result);
  }

  return {
    status: "ready",
    strategy: selected.some((result) => "semantic_score" in result) ? "vectorize_hybrid" : "local_fallback",
    vectorize_status: semanticRetrieval?.status ?? "not_configured",
    vectorize_index: semanticRetrieval?.index ?? null,
    query,
    document_type: "any",
    total_matches: Math.max(lexical.total_matches, vectorMatches.length),
    result_count: selected.length,
    semantic_result_count: selected.filter((result) => "semantic_score" in result).length,
    results: selected,
  };
}
...
export function getRelationships({ material_number, relationship_type = "all" }) {
  ...
  const relationships = [];
  if (source.record_type === "portable_balance") {
    ...
    const seen = new Set();
    for (const inbound of inboundRelationshipsByMaterial.get(String(source.material_number)) ?? []) {
      if (!requestedTypes.includes(inbound.relationship_type)) continue;
      const related = byMaterial.get(inbound.source_material_number);
      const key = `${inbound.relationship_type}|${inbound.source_material_number}`;
      if (seen.has(key)) continue;
      seen.add(key);
      relationships.push({
        direction: "inbound",
        relationship_type: inbound.relationship_type,
        related_material_number: inbound.source_material_number,
        resolution_status: related ? "resolved" : "needs_source",
        related_item: related ? compactRecord(related) : null,
        source_field: `relationships.${inbound.relationship_type}`,
      });
    }
  }
  return {
    status: "found",
    source: source.record_type === "portable_balance"
      ? { ...coreProduct(source), documents: source.documents }
...
function mentionedProductIdentifiers(text, contextMaterials = []) {
  ...
  for (const record of records) {
    ...
    if (normalizedText.includes(` ${normalize(material)} `)) found.add(material);
  }
  for (const product of products) {
    const model = normalize(product.model);
    if (model && normalizedText.includes(` ${model} `)) {
      found.add(String(product.material_number));
    }
  }
...
function groundedMaterialNumbers(...values) {
  const found = new Set();
  const visit = (value) => {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (/^(?:material_number|related_material_number)$/.test(key)) {
        const material = String(child ?? "");
        if (byMaterial.has(material)) found.add(material);
      }
      visit(child);
    }
  };
...
function compactRelationshipResult(result) {
  const sourceRecord = result.source ? byMaterial.get(String(result.source.material_number)) : null;
  return {
    status: result.status,
    source: sourceRecord
      ? sourceRecord.record_type === "portable_balance" ? coreProduct(sourceRecord) : compactRecord(sourceRecord)
      : null,
    ...
    relationships: (result.relationships ?? []).map((relationship) => ({
      ...
      related_item: relationship.related_item ? {
        model: relationship.related_item.model,
...
function deriveSelectionCriteria(question, interpretation = interpretCustomerQuestion(question)) {
  const text = String(question);
  ...
  const families = Object.keys(catalog.metadata.family_counts ?? {})
    ...
    .slice(0, 4);
  const batteryRequired = interpretation.derived_selection.battery_required === true
    || /battery[- ]?(?:powered|power|operation)|run(?:s|ning)? on batter|with batter/i.test(text)
    ? true
    : null;
  const applications = interpretation.derived_selection.applications;
  const usageContext = interpretation.derived_selection.usage_context;
  const searchTerms = interpretation.derived_selection.search_terms;
  const legalForTrade = interpretation.derived_selection.legal_for_trade;
  const criteriaApplied = [minimumCapacity, maximumCapacity, maximumReadability, batteryRequired, legalForTrade].some((value) => value !== null)
    || families.length > 0
 ...
 *     status?: string,
 *     index?: string | null,
 *     matches?: Array<{ id?: string, score?: number, document_id?: string }>,
 *   } | null,
 * }} options
 */
export function buildGroundingBundle({ question, sessionContext = [], semanticRetrieval = null }) {
  const interpretation = interpretCustomerQuestion(question);
  const expandedQuestion = interpretation.expanded_query;
  ...
  const wantsRelationships = /accessor|spare|part|replacement|related|connect|compatible|compatibility|\bfit(?:s)?\b|\bworks?\s+with\b/i.test(question);
  ...
  const relatedSearch = wantsRelatedItems && identifiers.length === 0
    ? searchCatalog({ query: expandedQuestion, record_type: "related_item", limit: 20 })
    : { query: question, record_type: "related_item", total_matches: 0, result_count: 0, results: [] };
  const topicFields = inspectCatalogFields({ query: expandedQuestion, limit: 20 });
  const relationships = wantsRelationships
    ...
    : [];
  const selectionCriteria = deriveSelectionCriteria(question, interpretation);
  ...
  const retrievalDocuments = completeExactRecordSupplied
    ...
    : semanticRetrievalDocuments({ query: expandedQuestion, semanticRetrieval });
  ...
  const compactRelatedSearch = compactSearch(relatedSearch, 20);
  const allowedMaterialNumbers = groundedMaterialNumbers(
    exact,
    deterministicSelection,
    nearestAlternatives,
    retrievalDocuments,
    compactProductSearch,
    compactRelatedSearch,
    relationships,
  );

  return {
    bundle_version: "sales-grounding-v9-vectorize",
    source_file: catalog.metadata.source_file,
    catalog_scope: getCatalogOverview(),
    user_question: question,
    allowed_material_numbers: allowedMaterialNumbers,
    interpreted_customer_language: interpretation,
    ...
    related_item_matches: compactRelatedSearch,
