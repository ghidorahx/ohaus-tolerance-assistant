...
export const DEFAULT_RETRIEVAL_DOCUMENTS = 8;
...
export function buildDeterministicLookupAnswer(question) {
  const text = String(question ?? "").trim();
  const identifiers = mentionedProductIdentifiers(text, []);
  if (identifiers.length !== 1) return null;
  if (/\b(?:compare|versus|vs\.?|difference|which|recommend|suggest|best|find|accessor|spare|replacement|related|compatible|feature|application|dimension|all|everything|complete|full|tell me|why|how)\b/i.test(text)) {
    return null;
  }

  const requestedFields = [
    { key: "capacity", path: "specifications.maximum_capacity", label: "maximum capacity", pattern: /\b(?:maximum )?capacit(?:y|ies)\b/i },
    { key: "readability", path: "specifications.readability", label: "readability", pattern: /\breadability\b/i },
    { key: "power", path: "specifications.power", label: "power", pattern: /\b(?:power|powered|power supply)\b/i },
    { key: "battery_life", path: "specifications.battery_life", label: "battery life", pattern: /\bbattery life\b/i },
  ].filter((field) => field.pattern.test(text));
  if (requestedFields.length === 0) return null;

  const resolution = resolveIdentifier(identifiers[0]);
  if (resolution.status !== "found" || resolution.record.record_type !== "portable_balance") return null;
  const record = resolution.record;
  const available = requestedFields.flatMap((field) => {
    const value = displayValue(getPath(record, field.path));
    return value === null ? [] : [{ ...field, value }];
  });
  const missing = requestedFields.filter((field) => !available.some((item) => item.key === field.key));
  const materialNumber = String(record.material_number);
  const model = String(record.model ?? record.product_name ?? materialNumber);
  const description = available.map((field) => `${field.label}: ${field.value}`).join(" · ");

  return {
    answer: missing.length > 0
      ? "Here are the requested specifications available in the loaded catalog."
      : "Here are the requested catalog specifications.",
    answer_items: [{
      identifier: materialNumber,
      label: model,
      description: description || "The requested specifications are not available in the loaded catalog.",
    }],
    status: available.length > 0 ? "answered" : "not_in_source",
    confidence: "high",
    intent: "lookup",
    materials: [materialNumber],
    evidence: available.map((field) => ({ material_number: materialNumber, field: field.path })),
    unresolved_items: missing.map((field) => `${field.label} is not available in the loaded catalog`),
    follow_up_suggestions: [],
    context_summary: `The user asked for exact catalog specifications for ${model}, material ${materialNumber}.`,
...
export function buildGroundingBundle({ question, sessionContext = [] }) {
  ...
  const asksForBroadCoverage = /\b(?:all models|all balances|all products|every model|every balance|everything|complete catalog|full catalog)\b/i.test(question);
  const retrievalLimit = asksForBroadCoverage
    ? MAX_RETRIEVAL_DOCUMENTS
    : selectionCriteria ? 6 : DEFAULT_RETRIEVAL_DOCUMENTS;
  const relatedSearch = wantsRelatedItems && identifiers.length === 0
    ? searchCatalog({ query: expandedQuestion, record_type: "related_item", limit: asksForBroadCoverage ? 20 : 12 })
    : { query: question, record_type: "related_item", total_matches: 0, result_count: 0, results: [] };
  const topicFields = inspectCatalogFields({ query: expandedQuestion, limit: asksForBroadCoverage ? 20 : 10 });
  ...
  const structuredExactRecordSupplied = identifiers.length > 0;
  const retrievalDocuments = structuredExactRecordSupplied
    ? {
      status: "ready",
      query: question,
      document_type: "any",
      ...
      result_count: 0,
      results: [],
      note: completeExactRecordSupplied
        ? "Complete exact structured record supplied; descriptive retrieval documents are unnecessary for this request."
        : "Exact structured record supplied; descriptive retrieval documents are unnecessary for this request.",
    }
  ...
  }, identifiers.length > 0 || deterministicSelection ? 0 : asksForBroadCoverage ? 20 : DEFAULT_RETRIEVAL_DOCUMENTS);

  return {
    bundle_version: "sales-grounding-v9",
    ...
    related_item_matches: compactSearch(relatedSearch, asksForBroadCoverage ? 20 : 12),
    relationship_results: relationships,
    topic_field_matches: {
      query: topicFields.query,
      fields: topicFields.fields.slice(0, asksForBroadCoverage ? 20 : 10).map((field) => ({
        path: field.path,
