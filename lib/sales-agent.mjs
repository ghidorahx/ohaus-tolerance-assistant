...
export function outputTokenLimitForQuestion(question) {
  ...
  if (/\b(?:compatible|compatibility|fit|fits|accessories|spare parts?|replacement parts?|related items?)\b|\bworks?\s+with\b/.test(query)) {
    return 8_000;
  }
...
function canRetryForCompleteList(question) {
  return /\b(?:all|every|complete|list|compatible|compatibility|fit|fits|accessories|spare parts?|replacement parts?|related items?)\b|\bworks?\s+with\b/i.test(String(question ?? ""));
}
