...
export async function retrieveMasterCatalog({
  ...
  const initialSettlements = await Promise.allSettled([
    resolveQuestionIdentifiers({ question: intentQuery, identifiers, contextMaterials, db, versionId: version.version_id }),
  ...
  const strategy = retrievalStrategy(resolved.records.length, lexical, retrievalNumeric, semantic, catalogListing);
