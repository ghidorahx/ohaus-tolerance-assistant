...
function buildFtsQuery(question) {
  const tokens = searchTokens(question, 24);
  return tokens.map((token) => `"${token.replaceAll('"', '""')}"`).join(" OR ");
}

function expandedRetrievalQuestion(question) {
  const text = String(question ?? "").trim();
  const additions = [];
  const add = (...terms) => additions.push(...terms);
  if (/\bbreak(?:ing)?\s+open\s+cells?\b|\bcell(?:ular)?\s+disruption\b/i.test(text)) add("cell lysis");
  if (/\bhomogeni[sz](?:e|es|ed|ing)\b/i.test(text)) add("homogenization");
  if (/\bsamples\b/i.test(text)) add("sample");
  if (/\bcramped\b/i.test(text)) add("compact", "tight");
  if (/\bbiosafety\s+cabinet\b/i.test(text)) add("biosafety cabinets");
...
function isDisplayValueWrapper(value) {
  ...
  return companionKeys.length <= 6
    && companionKeys.every((key) => /^(?:value|secondary_value|unit|secondary_unit|seconds|minutes|hours|raw|d)$/i.test(key));
}
...
async function exactAlternativeModel(db, versionId, identifier) {
  ...
  const rows = await queryAll(db, `
    ...
    JOIN master_catalog_versions AS v ON v.version_id = m.version_id
    WHERE a.version_id = ?
      AND a.field_key >= 'alternative_model'
      AND a.field_key < 'alternative_model\uffff'
...
const RELATIONSHIP_SUBJECT_PATTERNS = Object.freeze([
  /\b(?:listed|available)\s+for\s+(.+?)(?:[?!;]|$)/i,
  /\b(?:linked|related|connected)\s+to\s+(.+?)(?:[?!;]|$)/i,
]);

function relationshipSubjects(question) {
  const text = String(question ?? "");
  return RELATIONSHIP_SUBJECT_PATTERNS.flatMap((pattern) => {
    const match = text.match(pattern);
    const subject = match?.[1]?.replace(/^["'`\s]+|["'`\s]+$/g, "").trim();
    if (!match || !subject) return [];
    const offset = match[0].indexOf(match[1]);
    const start = (match.index ?? 0) + Math.max(0, offset);
    return [{ subject, start, end: start + match[1].length }];
  });
}

function catalogLikeRelationshipSubject(subject) {
  const measurement = /^-?\d[\d,.]*(?:kg|kilograms?|g|grams?|mg|milligrams?|lb|lbs|pounds?|oz|ounces?|rpm|r\/min|°?c|celsius|°?f|fahrenheit|mm|millimeters?|cm|centimeters?|m|meters?|inches?|inch|in|%|percent|v|volts?|w|watts?|hz)$/i;
  const tokens = String(subject ?? "").match(/[A-Za-z0-9][A-Za-z0-9._/-]*/g) ?? [];
  return tokens.some((token) => /[A-Za-z]/.test(token) && /\d/.test(token) && !measurement.test(token));
}
...
function inferredIdentifierCandidates(question) {
  ...
  const candidates = [];
  const labeledAlias = text.match(/\bmodel\s+alias\s+(.*?)(?:[?!;]|$)/i)?.[1]
    ?.replace(/^(?:is|#|:)\s*/i, "")
    .replace(/^["'`\s]+|["'`\s]+$/g, "")
    .trim();
  if (labeledAlias && labeledAlias.length >= 3 && labeledAlias.length <= 96 && /\d/.test(labeledAlias)) {
    candidates.push(labeledAlias);
  }
  if (requestedMasterRelationshipTypes(text).length > 0) {
    for (const { subject } of relationshipSubjects(text)) {
      if (subject && subject.length >= 3 && subject.length <= 160 && /\d/.test(subject)) {
        candidates.push(subject);
        break;
      }
    }
  }
  for (const match of text.matchAll(/\b(?:alternative\s+model(?:\s+number)?|model\s+(?:number|no)|part\s+number|sku|material\s+number|item\s+number)\s*(?:is|#|:)?\s*([A-Za-z0-9][A-Za-z0-9._/-]{2,})\b/gi)) {
    if (/\d/.test(match[1])) candidates.push(match[1]);
  }
  for (let index = 0; index < rawTokens.length; index += 1) {
    const token = rawTokens[index].replace(/^[._/-]+|[._/-]+$/g, "");
    const followedByMeasurementUnit = /^\d{6,12}$/.test(token)
      && /^(?:kg|kilograms?|g|grams?|mg|milligrams?|lb|lbs|pounds?|oz|ounces?|rpm)$/i.test(
        String(rawTokens[index + 1] ?? "").replace(/^[._/-]+|[._/-]+$/g, ""),
      );
    const modelLike = /^[A-Za-z]/.test(token) && /\d/.test(token) && token.length >= 3;
    const blockedDomainToken = /^(?:ip\d+|rs\d+|cl\d+)$/i.test(token);
    if ((/^\d{6,12}$/.test(token) && !followedByMeasurementUnit) || (modelLike && !blockedDomainToken)) {
      candidates.push(token);
      if (rawTokens[index + 1] && /\d/.test(rawTokens[index + 1])) candidates.push(`${token} ${rawTokens[index + 1]}`);
    }
  }
  const blockedPrefixes = new Set([
    "about", "above", "ac", "around", "astm", "at", "below", "class", "dc", "ip", "least",
    "max", "maximum", "min", "minimum", "most", "oiml", "over", "ph", "rs", "under", "usb",
  ]);
  for (let end = 1; end < rawTokens.length; end += 1) {
    ...
    for (const width of [1, 2]) {
      ...
      if (prefixes.some((token) => !/^[a-z]+$/.test(token) || blockedPrefixes.has(token) || FTS_STOP_WORDS.has(token))) continue;
...
function usesCatalogContextReference(question) {
...
async function resolveQuestionIdentifiers({ question, identifiers, contextMaterials, db, versionId }) {
  ...
  const usesReference = requested.length === 0 && usesCatalogContextReference(question);
  if (usesReference) {
...
function comparableCategoryText(value) {
  return normalizeCatalogIdentifier(String(value ?? "").replace(/[™®]/g, " "))
    .split(" ")
    .filter(Boolean)
    .map((token) => {
...
function locallyFilterCatalogListing(listing, eligibleMaterials) {
  ...
  const incomplete = Boolean(listing.truncated);
  return {
    ...listing,
    status: incomplete ? "fallback" : items.length > 0 ? "ready" : "no_results",
    ...(incomplete ? { reason: "numeric_category_filter_incomplete" } : {}),
    total_count: incomplete ? 0 : items.length,
    returned_count: incomplete ? 0 : items.length,
    ...
    numeric_filter_applied: !incomplete,
  };
}
...
async function lexicalSearch({ question, db, versionId, topK }) {
  ...
  const ftsQuery = buildFtsQuery(expandedRetrievalQuestion(question));
...
const NUMERIC_VALUE_SOURCE = String.raw`(?<![A-Za-z0-9-])-?\d[\d,]*(?:\.\d+)?`;
...
export function parseMasterNumericConstraints(question) {
  const text = String(question ?? "");
  const ignoredRelationshipSubjectSpans = requestedMasterRelationshipTypes(text).length > 0
    ? relationshipSubjects(text)
      .filter(({ subject }) => catalogLikeRelationshipSubject(subject))
  ...
  for (const match of text.matchAll(NUMERIC_RANGE)) {
    ...
    constraints.push({
      ...
      field_scope: field.name === "capacity" && /\bmaximum\s+capacity\b/i.test(text) ? "maximum_capacity" : null,
      range_group: `range_${rangeGroup}`,
      comparator: "at_least",
      value: lower,
      unit: lowerUnit,
      text: match[0],
  ...
  for (const match of text.matchAll(NUMERIC_MEASUREMENT)) {
    ...
    if (!match[1] && ignoredRelationshipSubjectSpans.some(([start, end]) => measurementIndex >= start && measurementIndex < end)) continue;
...
function numericFieldPredicate(constraint, alias = "a") {
  ...
  const strictMaximumCapacity = constraint.field === "capacity" && constraint.field_scope === "maximum_capacity";
  const keys = [...new Set(strictMaximumCapacity
    ? ["maximum_capacity_imp", "maximum_capacity_metric", "maximum_capacity_rotor"]
    : group?.keys ?? [])];
  const prefixes = [...new Set(strictMaximumCapacity ? ["maximum_capacity_"] : group?.prefixes ?? [])];
  const clauses = [];
  const parameters = [];
  if (keys.length > 0) {
...
function numericEligibilityPredicate(constraints, materialAlias = "m") {
  ...
  for (const [index, constraint] of constraints.entries()) {
    const key = constraint.range_group ? `range:${constraint.range_group}` : `single:${index}`;
    const group = groups.get(key) ?? [];
    group.push(constraint);
    groups.set(key, group);
  }
  for (const [index, groupedConstraints] of [...groups.values()].entries()) {
    const alias = `numeric_${index}`;
    const [constraint] = groupedConstraints;
    const fieldPredicate = numericFieldPredicate(constraint, alias);
    const valuePredicates = groupedConstraints.map((item) => nativeNumericSql(item, alias));
    clauses.push(`EXISTS (
      SELECT 1
      ...
      WHERE ${alias}.version_id = ${materialAlias}.version_id
        ...
        ${valuePredicates.map((predicate) => `AND (${predicate.clause})`).join("\n        ")}
    )`);
    parameters.push(...fieldPredicate.parameters, ...valuePredicates.flatMap((predicate) => predicate.parameters));
  }
...
async function semanticSearch({ question, ai, index, namespace, topK, minimumScore }) {
  ...
  try {
    const response = await ai.run(MASTER_EMBEDDING_MODEL, {
      ...
      text: [expandedRetrievalQuestion(question).split(/\s+/).slice(0, 420).join(" ").slice(0, 1_800)],
...
function exactChunkQueryTerms(question) {
  ...
  const expansions = [
    ...
    [/capacity|(?:how\s+much[^?]{0,48}\bweigh)|weigh(?:s|ing)?\s+(?:at\s+most|up\s+to)|(?:maximum|max|weight)\s+(?:load|weight)/i, ["performance", "capacity", "maximum", "load", "weight"]],
    [/readability|resolution|increment|division|smallest\s+(?:displayed\s+)?(?:amount|change|increment|step)/i, ["performance", "readability", "resolution", "increment"]],
    [/stabili[sz](?:e|es|ed|ing|ation)|settle|settling/i, ["performance", "stabilization", "settling"]],
    [/speed|rpm|r\/min/i, ["performance", "speed", "rpm"]],
    [/measurement\s+range|\bph\s+range|conductivity\s+range/i, ["performance", "measurement", "range", "ph", "conductivity"]],
    [/accuracy|repeatability|linearity/i, ["compliance", "performance", "accuracy", "repeatability", "linearity"]],
    [/temperature|humidity|environment/i, ["environment", "temperature", "humidity"]],
    [/compliance|certificate|approval|legal|metrology/i, ["compliance", "certificate", "approval", "metrology", "legal"]],
    [/ingress|\bip\s*rating|protection\s+rating/i, ["environment", "protection", "rating", "ip"]],
    [/\b(?:application|mode|parts?\s+counting|checkweighing|percent\s+weighing)\b/i, ["applications", "application", "weigh", "count", "percent"]],
    [/\b(?:battery|power|adapter|mains|voltage|ac|dc)\b/i, ["performance", "power", "battery", "adapter", "mains", "voltage", "ac", "dc"]],
  ];
...
export function selectCompactEvidence({ question, materials, chunks, attributes = [], limit = 16, includeInternalFields = false }) {
  ...
  // Reuse the technical-intent expansions that rank exact material chunks so
  // paraphrases such as "weigh at most" and "stabilize" also prioritize the
  // corresponding source fields inside a compact evidence budget.
  const queryTokens = exactChunkQueryTerms(question);
...
export function requestedMasterRelationshipTypes(question) {
  const types = [];
  const replacementParts = /replacement[ -]?parts?/i.test(question);
  if (/spare[ -]?parts?/i.test(question) || replacementParts) types.push("spare_parts");
  ...
  else if (types.length === 0 && /\b(?:compatible|compatibility)\b|\bfit(?:s)?\b|\bworks?\s+with\b/i.test(question)) types.push("all");
...
function requestedRelationshipDirections(question) {
  ...
  const asksForward = /\b(?:accessories|spare[ -]?parts?|replacement[ -]?parts?|services?)\b[^?]{0,60}\b(?:for|of|with)\b/i.test(question);
...
async function expandRelationships({ question, db, versionId, materialNumbers, limit }) {
  const types = requestedMasterRelationshipTypes(question);
...
function retrievalStrategy(exactCount, lexical, numeric, semantic, catalogListing = null) {
  if (catalogListing?.requested && ["ready", "no_results"].includes(catalogListing.status)) return "catalog_scope";
...
export function buildMasterCatalogPromptContext(bundle) {
  ...
  return {
    ...
    relationships: (bundle.relationships ?? []).slice(0, 80).map((relationship) => ({
...
export async function retrieveMasterCatalog({
  ...
  const intentQuery = String(rawQuestion ?? question ?? "").trim() || query;
  ...
  const warnings = [];
  const inferredIdentifiers = inferredIdentifierCandidates(intentQuery);
  ...
  const contextualIdentifierLookup = !explicitIdentifierLookup
    ...
    && /\b(?:it|its|that|this|those|these|them)\b/i.test(intentQuery)
  ...
  const parsedNumericConstraints = parseMasterNumericConstraints(intentQuery);
  const d1Channels = [];
  ...
  const resolutionRequest = () => resolveQuestionIdentifiers({
    question: intentQuery,
    identifiers,
    contextMaterials,
    db,
    versionId: version.version_id,
  });
  const broadRetrievalRequests = () => [
  ...
  let broadRetrievalSkipped = false;
  if (identifierLookupAttempted && parsedNumericConstraints.length === 0) {
    const [resolutionSettlement] = await Promise.allSettled([resolutionRequest()]);
    const resolution = resolutionSettlement.status === "fulfilled" ? resolutionSettlement.value : null;
    const resolutionStatuses = (resolution?.resolutions ?? []).map((item) => item.status);
    // Inferred candidates deliberately over-generate (for example, both a
    // spaced model alias and its numeric suffix). One authoritative match is
    // enough to stay on the exact path; caller-supplied identifiers must all
    // resolve so an explicitly requested comparison is never shortened.
    const deterministicallyResolved = resolutionStatuses.some((status) => ["found", "ambiguous"].includes(status))
      && (!explicitIdentifierLookup || resolutionStatuses.every((status) => ["found", "ambiguous"].includes(status)));
    if (deterministicallyResolved) {
      broadRetrievalSkipped = true;
      const fulfilled = (value) => ({ status: "fulfilled", value });
      initialSettlements = [
        resolutionSettlement,
        fulfilled({ status: "skipped", reason: "identifier_resolved", matches: [] }),
        fulfilled({ status: "skipped", reason: "identifier_resolved", constraints: [], eligible_materials: [], matches: [] }),
        fulfilled({ status: "skipped", reason: "identifier_resolved", matches: [] }),
        fulfilled({ ...EMPTY_CATALOG_LISTING }),
      ];
    } else {
      initialSettlements = [resolutionSettlement, ...await Promise.allSettled(broadRetrievalRequests())];
    }
  } else {
    initialSettlements = await Promise.allSettled([resolutionRequest(), ...broadRetrievalRequests()]);
  }
  const lexicalLookupAttempted = !broadRetrievalSkipped && Boolean(buildFtsQuery(query));
  ...
  if (hardNumericFilter && catalogListing.kind === "materials_by_category") {
    try {
      ...
      noteD1Channel("numeric_catalog_listing", true);
    } catch (error) {
      ...
      noteD1Channel("numeric_catalog_listing", false);
  ...
  const numericCatalogListingUnavailable = hardNumericFilter
    && catalogListing.kind === "materials_by_category"
    && catalogListing.status === "fallback";
  const categoryNumericScope = hardNumericFilter
    && catalogListing.kind === "materials_by_category"
    && ["ready", "no_results"].includes(catalogListing.status);
  const categoryNumericMaterials = categoryNumericScope
    ? [...new Set((catalogListing.items ?? []).map((item) => String(item.material_number ?? "")).filter(Boolean))]
    : [];
  const scopedNumericEligible = categoryNumericScope ? new Set(categoryNumericMaterials) : numericEligible;
  const retrievalNumeric = categoryNumericScope ? {
    ...numeric,
    eligible_materials: categoryNumericMaterials,
    matches: (numeric.matches ?? []).filter((match) => scopedNumericEligible.has(String(match.material_number ?? ""))),
  } : numeric;

  const fused = reciprocalRankFuse([
    { name: "lexical", weight: 1.1, matches: lexical.matches },
    { name: "numeric", weight: 1.35, matches: retrievalNumeric.matches },
  ...
  const selectedChunkLimit = exactMaterialCount === 1 && !exhaustiveExactLookup
    ? Math.min(3, maximumChunks)
    : maximumChunks;
  const categoryEvidenceMaterials = categoryNumericMaterials.slice(
    0,
    Math.max(1, Math.floor(selectedChunkLimit / 2)),
  );
  const chunkSettlements = await Promise.allSettled([
    ...
    chunksByIds(db, version.version_id, retrievalNumeric.matches.map((match, index) => ({
      ...match,
      score: 1,
      sources: ["numeric"],
  ...
  if (chunkSettlements[3].status === "fulfilled") categoryNumericChunks = chunkSettlements[3].value;
  else warnings.push(`Category numeric chunk hydration unavailable: ${errorMessage(chunkSettlements[3].reason)}`);
  ...
  if (retrievalNumeric.matches.length > 0) noteCandidateHydration("numeric_chunk_hydration", chunkSettlements[2].status === "fulfilled");
  if (categoryEvidenceMaterials.length > 0) noteCandidateHydration("category_numeric_chunk_hydration", chunkSettlements[3].status === "fulfilled");
  ...
  const eligibleRankedChunks = hardNumericFilter
    ? rankedChunks.filter((chunk) => scopedNumericEligible.has(String(chunk.material_number ?? "")))
    : rankedChunks;
  ...
  const chunks = selectChunks(
    [...exactChunks, ...categoryNumericChunks],
    numericChunks,
  ...
  const exactRelationshipMaterials = [...new Set(
    resolved.records.map((record) => String(record.material_number)),
  )];
  const requestedMaterialNumbers = (resolved.requested ?? [])
    .map((identifier) => String(identifier).trim())
    .filter((identifier) => /^\d{6,12}$/.test(identifier));
  // An exact relationship request must spend its complete row budget on the
  // requested material(s). Ranked lexical/semantic neighbors are useful only
  // as a fallback when identifier resolution produced no exact record.
  const relationshipMaterials = (exactRelationshipMaterials.length > 0
    ? exactRelationshipMaterials
    : [...new Set([...requestedMaterialNumbers, ...expansionMaterials])]
  ).slice(0, 8);
  const documentMaterials = resolved.records.length > 0
    ? [...new Set(resolved.records.map((record) => String(record.material_number)))].slice(0, 6)
    : expansionMaterials;
  let relationshipExpansion = { requested: false, results: [], truncated: false, warnings: [] };
  const documentExpansionRequested = documentsRequested(intentQuery);
  let documentExpansion = {
    requested: documentExpansionRequested,
    status: documentExpansionRequested ? "unavailable" : "skipped",
    results: [],
    truncated: false,
  };
  let attributes = [];
  const enrichmentSettlements = await Promise.allSettled([
    ...
    expandDocuments({
      ...
      db,
      versionId: version.version_id,
      materialNumbers: documentMaterials,
  ...
  if (enrichmentSettlements[1].status === "fulfilled") {
    documentExpansion = { ...enrichmentSettlements[1].value, status: documentExpansionRequested ? "ready" : "skipped" };
  }
  ...
  const resolvedIdentifierCount = (resolved.resolutions ?? [])
    .filter((item) => ["found", "ambiguous"].includes(item.status)).length;
  const strategy = retrievalStrategy(Math.max(resolved.records.length, resolvedIdentifierCount), lexical, retrievalNumeric, semantic, catalogListing);
  const hasResults = resolvedIdentifierCount > 0
  ...
  const d1RetrievalUnavailable = numericCatalogListingUnavailable || (!hasResults
    && ((d1Channels.length > 0 && succeededD1Channels.length === 0) || candidateHydrationUnavailable));
  const provenNoResults = hardNumericFilter
    && catalogListing.requested
    && catalogListing.status === "no_results";
  const result = {
    bundle_version: MASTER_CATALOG_BUNDLE_VERSION,
    status: d1RetrievalUnavailable
      ...
      : hasResults
        ? "ready"
        : provenNoResults || strategy === "none" ? "no_results" : "ready",
    ...
    documents_requested: documentExpansion.requested,
    documents_status: documentExpansion.status,
