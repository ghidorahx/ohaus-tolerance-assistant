...
function inferredIdentifierCandidates(question) {
  ...
  const candidates = [];
  for (const match of String(question ?? "").matchAll(/\b(?:alternative\s+model(?:\s+number)?|model\s+(?:number|no)|part\s+number|sku|material\s+number|item\s+number)\s*(?:is|#|:)?\s*([A-Za-z0-9][A-Za-z0-9._/-]{2,})\b/gi)) {
...
export async function retrieveMasterCatalog({
  ...
  const documentExpansionRequested = documentsRequested(query);
