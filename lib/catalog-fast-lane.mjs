...
export const CATALOG_FAST_LANE_LIST_LIMIT = 80;
const CATALOG_FAST_LANE_MATERIAL_LIMIT = 96;
...
const FIELD_CONCEPTS = Object.freeze([
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(?:which|what) (?:models?|products?|items?|balances?|scales?|instruments?)\b[^?]{0,80}\b(?:compatible|fit|fits|works?\s+with)\b/i,
    ],
...
function masterRelationshipAnswer(retrieval, interpretation, primaryModel) {
  ...
  const compatibleModelsRequested = interpretation.concepts.some((concept) => concept.id === "compatible_models");
  for (const row of rows) {
    ...
    const relationshipType = String(row.relationship_type ?? interpretation.relationship_type ?? "related_item");
    const type = inbound && compatibleModelsRequested ? "compatible model" : relationshipType.replaceAll("_", " ");
    ...
    const key = `${sourceMaterial}|${relationshipType}`;
    const group = evidenceGroups.get(key) ?? [];
    group.push(`${identifier} — ${label}`);
    evidenceGroups.set(key, group);
    if (items.length >= CATALOG_FAST_LANE_LIST_LIMIT) break;
  }
  const uniqueRowCount = new Set(rows.map((row) => String(
    row.related_material_number
      ?? (row.direction === "inbound" ? row.source_material_number : row.target_material_number)
      ?? "",
  )).filter(Boolean)).size;
  const displayTruncated = Boolean(retrieval.relationships_truncated) || uniqueRowCount > items.length;
  const displayedCount = retrieval.relationships_truncated ? `${Math.max(items.length, uniqueRowCount)}+` : String(uniqueRowCount);
  const sourceLabel = exactRecords.map(recordLabel).join(", ");
  const relationshipLabel = compatibleModelsRequested && rows.some((row) => row.direction === "inbound")
    ? "compatible models"
    : String(interpretation.relationship_type ?? "related items").replaceAll("_", " ");
  ...
  return fastMetadata(baseAnswer({
    answer: items.length > 0
      ? `${displayedCount} ${relationshipLabel} ${displayedCount === "1" ? "is" : "are"} listed for ${sourceLabel}${displayTruncated ? `; showing the first ${items.length}` : ""}.`
      : `No ${relationshipLabel} are listed for ${sourceLabel} in the loaded Excel catalog.`,
    answer_items: items,
    status: "answered",
    confidence: displayTruncated ? "medium" : "high",
    intent: "relationship",
    materials: [...new Set([...sourceMaterials, ...items.map((item) => item.identifier)])].slice(0, CATALOG_FAST_LANE_MATERIAL_LIMIT),
    evidence,
...
function legacyRelationshipAnswer(grounding, interpretation, primaryModel) {
  ...
  const compatibleModelsRequested = interpretation.concepts.some((concept) => concept.id === "compatible_models");
  let hasInboundRelationships = false;
  for (const group of groups) {
    ...
    for (const relationship of relationships) {
      ...
      if (inbound) hasInboundRelationships = true;
      ...
      const type = inbound && compatibleModelsRequested
        ? "compatible model"
        : String(relationship.relationship_type ?? "related item").replaceAll("_", " ");
  ...
  const relationshipLabel = compatibleModelsRequested && hasInboundRelationships
    ? "compatible models"
    : String(interpretation.relationship_type ?? "related items").replaceAll("_", " ");
  const displayedItems = items.slice(0, CATALOG_FAST_LANE_LIST_LIMIT);
  const displayTruncated = items.length > displayedItems.length;
  return fastMetadata(baseAnswer({
    answer: items.length > 0
      ? `${items.length} ${relationshipLabel} ${items.length === 1 ? "is" : "are"} listed for ${sourceLabels}${displayTruncated ? `; showing the first ${displayedItems.length}` : ""}.`
      : `No ${relationshipLabel} are listed for ${sourceLabels} in the loaded Excel catalog.`,
    answer_items: displayedItems,
    confidence: displayTruncated ? "medium" : "high",
    intent: "relationship",
    materials: [...new Set([...sourceMaterials, ...displayedItems.map((item) => item.identifier)])].slice(0, CATALOG_FAST_LANE_MATERIAL_LIMIT),
    evidence,
    unresolved_items: displayTruncated ? ["Additional relationships are not shown; narrow the relationship type to see a smaller set."] : [],
...
function catalogListingAnswer(retrieval, interpretation, primaryModel) {
  ...
  if (listing.kind === "materials_by_category") {
    const items = (listing.items ?? []).slice(0, CATALOG_FAST_LANE_LIST_LIMIT).map((item) => ({
